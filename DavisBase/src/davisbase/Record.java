...
public class Record {
    ...
    public Record(boolean noRowId, byte[] rowData) {
        if (noRowId == true) {
            this.rowid = -1;
            this.numOfColumns = rowData[0] & 0xFF;
            int pos = 1;
            int end = 1 + numOfColumns;
            ...
            System.out.println("Use Record(byte[]) for record entry with row id");
        }
    }
...
//convert_to_storage_format_executor
    ...
    public void setRowContent(int columnposition, String newvalue) {
         ...
         *
         */
        if(this.columnTypes.get(columnposition-1)>=0x0c){
            //if it's a string
            //get the size of the string container an compare to new
            int newstringlength=0x0c+newvalue.length();
            if(this.columnTypes.get(columnposition-1)<newstringlength){
                setColumnType(columnposition,(byte)newstringlength);
                SetRowID(-1);
            }
            else if(this.columnTypes.get(columnposition-1)>=newstringlength){
                //buffer the remaining string container with 0s in DataConversion (hopefully?)
            }
        }
        this.rowContents.set(columnposition, new String(newvalue));
    }

    public void showfullRecord(boolean showid) {//for 
        int start;
        if (showid == true) {
